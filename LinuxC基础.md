...
1. **预处理**：
   这一步主要是处理源代码中的预处理指令，比如`#include`、`#define`等。举个例子，如果你的代码中写了`#include <stdio.h>`，预处理阶段就会找到这个头文件，并把它的内容复制到你的源代码中。这样，后续编译时就能找到那些函数和变量的定义了。

   **把编写好的xx.c源代码⽣成我预处理过得 C 代码 xx.i。**

2. **编译**：
   经过预处理后，GCC会将源代码（现在包含了所有头文件的内容）转化为汇编语言。这一步是真正的“编译”过程，编译器会检查源代码中的语法错误、类型错误等，并将有效的代码转化为汇编指令。此时计算机还不能直接理解这些汇编指令。

   **把我们预处理过的代码⽣成我们的汇编代码 xx.s。**

3. **汇编**：
   接下来，汇编器会把上一步生成的汇编代码转换成机器代码，也就是二进制指令。这些指令是计算机能直接理解的。

   **把汇编代码⽣成我们的⽬标⽂件 xx.o。**

4. **链接**：
   生成的机器代码可能还包含一些对外部函数或变量的引用，比如你调用了`printf`函数。链接器的作用就是找到这些函数或变量在别的库或对象文件中的定义，并把它们链接到你的程序中。你可以想象成，你的程序是一个房子，但还缺少一些家具（比如`printf`函数）。链接器就是去家具店（库文件）里找到这些家具，并把它们搬到你的房子里。

   **把我们的⽬标⽂件⽣成我们的可执⾏⽂件。**

最后，经过这些步骤，GCC就会生成一个可执行文件。你可以直接运行这个文件，看到程序的输出结果。
...
- **函数的声明**：函数的声明（也被称为函数的前向声明或函数原型）就是告诉编译器函数的名称、返回类型以及参数列表（包括参数的类型和数量）。这样，编译器就知道在其他地方调用这个函数时是合法的。函数的声明通常出现在函数被调用之前，以便编译器能正确解析代码。
...
这里的`extern int myVariable;`告诉编译器：“嘿，编译器，我知道这个`myVariable`变量在某个地方被定义了，但我这里只是用一下，你帮我去找它吧。”
...
说明： 上述代码，程序从 main.c 开始运⾏，第⼀次运⾏运⾏到 head1.h 中，int x = 10 和 int y = 20 定义了⼀次，第⼆次 运⾏到 head2.h , ⽽ head2.h 中⼜运⾏了⼀次 head1.h，⼜定义了 int x = 10 和 int y = 20 一次，从⽽编译器会提示重复定义。
