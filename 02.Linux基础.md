...
```
所有依赖⽂件的集合，使⽤空格分开，如果在依赖⽂件中有多个重复的⽂件，会去除重复的依赖⽂件，只保留⼀份。
```
...
Ubuntu下可以从文本文件的每一行中截取指定内容的数据的命令是：
...
wc命令用于统计文件的行数、单词数或字节数，关于wc -I的说法是:
...
Linux文件权限一共10位长度， 分成3段， 第3段表示的内容是:
...
假设目录中存在若干文件flenum.txt (num表示文件编号，1~10)，以下可以挑选出编号为1、3、5的文件的通配指令参数是:
...
当使用mount进行设备或者文件系统挂载的时候，需要用到的设备名称位于目录:
