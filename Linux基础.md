...
在我们的 linux 中，所有的⼀切都是从根⽬录开始的。我们可以这么说，所有的设备都是⽂件。例如， ⼀块硬盘是⼀个⽂件，这个硬盘上的分区也是⼀个⽂件，声卡也是⼀个⽂件。 这种表示⼀个的⽂件叫做设备⽂件。设备⽂件⼀般来说保存在 / dev 这个⽬录中。 ⼀个硬盘是⼀个设备⽂件，⼀般是 / dev/sda（第⼀块）；⽽硬盘上的每⼀个分区也是设备⽂件，⽐ 如第⼀个分区叫做 / dev/sda1，第⼆个是 / dev/sda2。 假设你可以直接读写硬盘的设备⽂件，但是，你得到的是硬盘的原始数据，⼏乎是 0/1 这样的⼆进 制。因此，你可以直接像修改⽂件⼀般的修改硬盘的分区表等结构。⽽每⼀个分区也是⼀个设备⽂件， 同样，你得到的是原始数据。

但是，没有⼈可以⼿⼯修改 0101，来复制⽂件、删除⽂件。为了使你操作⾥⾯的数据，⽽不是原始 010101，就需要进⾏挂载。⽐如，你把 / dev/sda1 挂到了 / boot 上，此时，驱动程序就会将 01 的原 始数据转换成能操作的结构，并装到 / boot 上，此时，对 / boot 的修改同等于修改 / dev/sda1，只 不过你可以复制、删除等，⽽不是看到 0101 的原始数据。

挂载 ： 指的就是将设备⽂件中的顶级⽬录连接到 Linux 根⽬录下的某⼀⽬录（最好是空⽬录），访问 此⽬录就等同于访问设备⽂件 **。** 

说明： 并不是根⽬录下任何⼀个⽬录都可以作为挂载点，由于挂载操作会使得原有⽬录中⽂件被 隐藏，因此根⽬录以及系统原有⽬录都不要作为挂载点，会造成系统异常甚⾄崩溃，挂载点最好是新建 的空⽬录。
